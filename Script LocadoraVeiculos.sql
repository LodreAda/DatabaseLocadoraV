...
endereco varchar(100)
 ...
 idLocação int not null,
 idCliente int,
 IdPagamento int,
 dataInicio date,
 dataFim date
 );
 create table LocaçãoVeiculo(
 idLocação int,
 idVeiculo int
 );
 show tables;
...
insert into Manutenção  (idManutenção, idVeiculo, descriçao, dataMan, custo) values
...
insert into Pagamento (idPagamento, forma, dataPag, valorTotal, estado) values
...
insert into Locação (idLocação, idCliente, idPagamento, dataInicio, dataFim) values
...
insert into LocaçãoVeiculo (idLocação, idVeiculo) values
     (1, 2),
     (2, 3),
     (3, 5),
     (4, 8),
...
select * from LocaçãoVeiculo; 
select valorTotal from Pagamento where estado = 'Pago';
select v.modelo, v.marca, COUNT(lv.idLocação) as numero_de_alugueis
from Veiculo v
left join LocaçãoVeiculo lv on v.idVeiculo = lv.idVeiculo
left join Locação l on lv.idLocação = l.idLocação
group by v.modelo, v.marca
order by numero_de_alugueis desc;
select c.nome from Cliente c
join Locação l on c.idCliente = l.idCliente
